...
def apply_theme():
    ...
    tema_opciones = {"🌙 Modo Oscuro": "dark", "☀️ Modo Claro": "light", "🔄 Automático (Sistema)": "auto"}
    with st.sidebar:
        st.markdown("---")
        tema_seleccionado = st.selectbox("🎨 Tema de Interface", options=list(tema_opciones.keys()), index=0, key="theme_selector")
        ocultar_valores = st.checkbox("👁️ Ocultar Valores Monetarios", value=False, key="hide_monetary_values")
    return tema_opciones[tema_seleccionado], ocultar_valores

def format_money(value, hide_values=False):
    """Formatea valores monetarios respetando configuración"""
    if hide_values:
        return "💰 ****"
    if pd.isna(value):
        return "$0"
...
def apply_theme_styles_only(tema):
    """Solo aplica estilos CSS del tema sin crear elementos"""
    if tema in ["dark", "auto"]:
        st.markdown("""
        <style>
...
def main():
    ...
    # Aplicar tema
    tema, ocultar_valores = apply_theme()
    apply_theme_styles_only(tema)
